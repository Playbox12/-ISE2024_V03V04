...
void Thread_Lcd (void *argument) {
  MSGQUEUE_LCD_t colaMensajesLcd;
	
  
  while (1) {
    
		osMessageQueueGet(mid_MsgQueueLcd, &colaMensajesLcd, NULL, 0U);
		
    if(colaMensajesLcd.linea==1){
			positionL1=0;
      writeline1(colaMensajesLcd.bufferLcd);
			LCD_update();
    }
		if(colaMensajesLcd.linea==2){
			positionL2=0;
			writeline2(colaMensajesLcd.bufferLcd);
			LCD_update();
		}
